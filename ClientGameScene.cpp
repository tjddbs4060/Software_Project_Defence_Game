...
bool Client::init()
{
	...
	MenuItemSprite* menuSoul = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuSkip = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuGamble = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuUpgrade = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuMix = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuCapsule = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuHero = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuBoss = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	MenuItemSprite* menuFriend = MenuItemSprite::create(menuNormal, menuNormal, CC_CALLBACK_1(Client::onMenu, this));
	...
	schedule(schedule_selector(Client::zorder_assort));
	schedule(schedule_selector(Client::unit_atk_cooltime));
	schedule(schedule_selector(Client::unit_atk_monster));
	schedule(schedule_selector(Client::add_unit_queue));
	schedule(schedule_selector(Client::server_continue), 0.9f);
	schedule(schedule_selector(Client::atk_boss));

	auto listener = EventListenerTouchAllAtOnce::create();
	listener->onTouchesBegan = CC_CALLBACK_2(Client::onTouchesBegan, this);
	listener->onTouchesCancelled = CC_CALLBACK_2(Client::onTouchesCancelled, this);
	listener->onTouchesEnded = CC_CALLBACK_2(Client::onTouchesEnded, this);
	listener->onTouchesMoved = CC_CALLBACK_2(Client::onTouchesMoved, this);
...
void Client::addmonster(float dt)
{
...
void Client::selfRemover(Node* sender)
{
	sender->removeFromParentAndCleanup(true);
}

void Client::monsterRemover(Node* sender)
{
...
void Client::addmonster_death(Point pt, float scale, bool isboss)
{
	char szFile[64] = { 0, };
	...
	CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::selfRemover, this));
	Sequence* sequence = Sequence::create(animate, callfunc, NULL);

	sprite_death->runAction(sequence);
}

void Client::unitRemover(Node* sender)
{
	//조합하여 사라지는것
	//팔아서 사라지는것
	Unit* unit = NULL;
...
void Client::addunit(char* sprite_name, char* name, char* type, int number, float speed, float range, float damage, bool help = false)
{
	Size winSize = Director::getInstance()->getWinSize();
	char szFile[64] = { 0, };
...
void Client::addunit_mix(Point pt)
{

}

void Client::addunit_sell(Point pt)
{

}

float Client::calDistance(Point from, Point to)
{
	float x = from.x - to.x;
	float y = from.y - to.y;
...
void Client::unit_atk_monster(float dt)
{
	InforBoard* inforBoard = (InforBoard*)getChildByTag(TAG_UNIT)->getChildByTag(TAG_INFORBOARD);
...
void Client::unit_atk_cooltime(float dt)
{
	Unit* unit = NULL;
...
void Client::unit_atk_motion(Unit* unit, bool right)
{
	char szFile[64] = { 0, };
...
void Client::onTouchesBegan(const std::vector<Touch*>& touches, Event *event)
{
	touch = true;
...
void Client::onTouchesCancelled(const std::vector<Touch*>& touches, Event *event)
{
	touch = false;
	touch_unit = false;
...
void Client::allstop_motion(Node* sender)
{
	sender->stopAllActions();
}

void Client::onTouchesEnded(const std::vector<Touch*>& touches, Event *event)
{
	if (touch_unit == true)
	{
		SpriteFrameCache* frameCache = SpriteFrameCache::getInstance();
		...
		CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::allstop_motion, this));
		auto animation = Animation::create();
				...
				if (heroList->getLocation() == 1)
				{
					for (std::vector<Unit*>::iterator iterUnit = arr_unit.begin(); iterUnit != arr_unit.end(); iterUnit++)
					{
						...
						if (!strcmp(heroList->getType(), unit->getType()) && heroList->getCount() == unit->getCount())
						{
							...
							CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::removeChild_background_boss, this));
				...
				else if (heroList->getLocation() == 3)
				{
					...
					for (std::vector<Unit*>::iterator iterUnit = arr_help_send_unit.begin(); iterUnit != arr_help_send_unit.end(); iterUnit++)
					{
						...
						if (!strcmp(heroList->getType(), unit->getType()) && heroList->getCount() == unit->getCount())
						{
							...
							CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::removeChild_help_boss, this));
				...
				if (heroList->getLocation() == 2)
				{
					for (std::vector<Unit*>::iterator iterUnit = arr_boss_room_unit.begin(); iterUnit != arr_boss_room_unit.end(); iterUnit++)
					{
						...
						if (!strcmp(heroList->getType(), unit->getType()) && heroList->getCount() == unit->getCount())
						{
							...
							CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::removeChild_boss_background, this));
				...
				else if (heroList->getLocation() == 3)
				{
					...
					for (std::vector<Unit*>::iterator iterUnit = arr_help_send_unit.begin(); iterUnit != arr_help_send_unit.end(); iterUnit++)
					{
						...
						if (!strcmp(heroList->getType(), unit->getType()) && heroList->getCount() == unit->getCount())
						{
							...
							CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::removeChild_help_background, this));
				...
				if (heroList->getLocation() == 1)
				{
					...
					for (std::vector<Unit*>::iterator iterUnit = arr_unit.begin(); iterUnit != arr_unit.end(); iterUnit++)
					{
						...
						if (!strcmp(heroList->getType(), unit->getType()) && heroList->getCount() == unit->getCount())
						{
							...
							CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::removeChild_background_help, this));
				...
				else if (heroList->getLocation() == 2)
				{
					...
					for (std::vector<Unit*>::iterator iterUnit = arr_boss_room_unit.begin(); iterUnit != arr_boss_room_unit.end(); iterUnit++)
					{
						...
						if (!strcmp(heroList->getType(), unit->getType()) && heroList->getCount() == unit->getCount())
						{
							...
							CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::removeChild_boss_help, this));
...
void Client::add_unit_queue(float dt)
{
...
void Client::onTouchesMoved(const std::vector<Touch*>& touches, Event *event)
{
	MixBoard* mixBoard = (MixBoard*)getChildByTag(TAG_HERO)->getChildByTag(TAG_INTERFACE_MIX);
...
Point Client::map_out_check(Point point)
{
	Point temp = point;
...
void Client::monster_location_init(Sprite* sprite)
{
...
void Client::zorder_assort(float dt)
{
	char szFile[32] = { 0, };
	Unit* unit = NULL;
	Monster* monster = NULL;
	...
	if (summon_monster == 11 && monster_add_start == true)
	{
		char szFile[64];

		monster_add_start = false;
		...
		schedule(schedule_selector(Client::addmonster), 1.f);
		new_soul_1 = true;
		new_soul_2 = false;
	}
	else if (summon_monster == 0)
	{
		unschedule(schedule_selector(Client::addmonster));
	}
...
void Client::move_unit(Unit* unit, bool right)
{
	char szFile[64] = { 0, };
...
void Client::addlabel(char* name, int gold, int choice)
{
	Size winSize = Director::getInstance()->getWinSize();
	SpriteFrameCache* frameCache = SpriteFrameCache::getInstance();
	...
	CallFuncN* callfunc = CallFuncN::create(CC_CALLBACK_1(Client::labelRemover, this));
...
void Client::labelRemover(Node* sender)
{
	Sprite* sprite = NULL;
...
bool Client::rand_cal(float per)
{
...
void Client::onHttpRequestCompleted(cocos2d::network::HttpClient * sender, cocos2d::network::HttpResponse * response)
{
	...
	else if (!strcmp(compare, "time"))
	{
		...
		inforBoard->setTime(atof(time));

		if (atoi(stage) != inforBoard->getStage())
		{
			sound_stage();

			inforBoard->setStage(atoi(stage));
			summon_monster = 11;
			monster_add_start = true;
		}
	}
...
void Client::get_db_data(char * data, int port)
{
...
void Client::server_continue(float dt)
{
	InforBoard* inforBoard = (InforBoard*)getChildByTag(TAG_UNIT)->getChildByTag(TAG_INFORBOARD);

	char szFile[64] = { 0, };

	sprintf(szFile, "gettime/%s", getID());
	get_db_data(szFile, DEFENCEJS);
...
void Client::upgrade_update(char* up)
{
	float rate;
	float damage;
	int index;
...
void Client::GameOver()
{
	...
	unschedule(schedule_selector(Client::zorder_assort));
	unschedule(schedule_selector(Client::unit_atk_cooltime));
	unschedule(schedule_selector(Client::unit_atk_monster));
	unschedule(schedule_selector(Client::add_unit_queue));
	unschedule(schedule_selector(Client::server_continue));
...
void Client::create_boss(char* name, float hp, float def)
{
	char szFile[64] = { 0, };
...
void Client::removeChild_background_boss(Node* sender)
{
...
void Client::removeChild_background_help(Node* sender)
{
	//getChildByTag(TAG_BACKGROUND)->removeChild(sender, false);
	sender->setVisible(false);
}

void Client::removeChild_boss_help(Node* sender)
{
	//getChildByTag(TAG_MENU)->getChildByTag(TAG_MENU_BOSS)->getChildByTag(TAG_INTERFACE_BOSS)->removeChild(sender, false);
	sender->setVisible(false);
}

void Client::removeChild_boss_background(Node* sender)
{
	getChildByTag(TAG_MENU)->getChildByTag(TAG_MENU_BOSS)->getChildByTag(TAG_INTERFACE_BOSS)->removeChild(sender, false);
...
void Client::removeChild_help_background(Node* sender)
{
	//getChildByTag(TAG_BACKGROUND)->addChild(sender);
...
void Client::removeChild_help_boss(Node* sender)
{
	//getChildByTag(TAG_MENU)->getChildByTag(TAG_MENU_BOSS)->getChildByTag(TAG_INTERFACE_BOSS)->addChild(sender);
	if (sender->getParent()->getTag() == TAG_BACKGROUND)
	{
